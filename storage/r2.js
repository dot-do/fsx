 ...
 * @module storage/r2
 */
import { StorageError, createOperationContext, createOperationResult, withInstrumentation, } from './interfaces.js';
 ...
 */
function handleR2Error(error, path, operation) {
    const err = error;
    ...
    if (err.code === 'ERR_R2_RATE_LIMITED') {
        const storageError = new StorageError('ETIMEDOUT', `Rate limit exceeded: ${path}`, { path, operation, cause: err });
 ...
 */
async function _withRetry(fn, config, path, operation) {
    let lastError;
 ...
 */
async function withInstrumentationAndSize(hooks, operation, path, fn, getSize) {
...
export class R2Storage {
    ...
    async put(path, data, options) {
        ...
        return withInstrumentationAndSize(this.hooks, 'put', path, async () => {
            ...
            if (options?.md5) {
                customMetadata['x-md5'] = options.md5;
            }
            try {
     ...
     */
    async get(path) {
        return withInstrumentationAndSize(this.hooks, 'get', path, async () => {
            const key = this.key(path);
            try {
    ...
    async getRange(path, start, end) {
        return withInstrumentationAndSize(this.hooks, 'getRange', path, async () => {
     ...
     * ```
     */
    async getRangeSuffix(path, length) {
        return withInstrumentationAndSize(this.hooks, 'getRangeSuffix', path, async () => {
            const key = this.key(path);

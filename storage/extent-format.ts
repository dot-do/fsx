 ...
 * ```
 */
export function computeChecksum(data: Uint8Array): bigint {
 ...
 * ```
 */
export function buildExtent(
  pages: Map<number, Uint8Array>,
  pageSize: number,
  options?: BuildExtentOptions
): Uint8Array {
  // Validate pageSize
  if (pageSize <= 0) {
  ...
  for (const [index, data] of pages) {
    if (data.length !== pageSize) {
      throw new Error(
        `Page ${index} has size ${data.length}, expected ${pageSize}`
      )
    }
  }
 ...
 * ```
 */
export function parseExtentHeader(data: Uint8Array): ExtentHeader {
  if (data.length < EXTENT_HEADER_SIZE) {
    throw new Error(
      `Extent data too small: ${data.length} bytes, minimum is ${EXTENT_HEADER_SIZE}`
    )
  }
  ...
  if (magic !== EXTENT_MAGIC) {
    throw new Error(
      `Invalid extent magic: 0x${magic.toString(16)}, expected 0x${EXTENT_MAGIC.toString(16)}`
    )
  }
  ...
  if (version !== EXTENT_VERSION) {
    throw new Error(
      `Unsupported extent version: ${version}, expected ${EXTENT_VERSION}`
    )
  }
 ...
 * ```
 */
export function getPageBitmap(extent: Uint8Array): Uint8Array {
  const header = parseExtentHeader(extent)
  const bitmapSize = Math.ceil(header.pageCount / 8)

  if (extent.length < EXTENT_HEADER_SIZE + bitmapSize) {
    throw new Error(
      `Extent data too small for bitmap: ${extent.length} bytes, ` +
 ...
 * ```
 */
export function isPagePresent(extent: Uint8Array, pageIndex: number): boolean {
  const header = parseExtentHeader(extent)
...
export function extractPage(
  ...
  if (pageSize !== header.pageSize) {
    throw new Error(
      `Page size mismatch: requested ${pageSize}, extent has ${header.pageSize}`
    )
  }
  ...
  const dataOffset =
    EXTENT_HEADER_SIZE + bitmapSize + precedingPagesPresent * pageSize

  // Validate we have enough data
  if (extent.length < dataOffset + pageSize) {
    throw new Error(
      `Extent data truncated: expected at least ${dataOffset + pageSize} bytes, ` +
        `got ${extent.length}`
    )
  }
 ...
 * ```
 */
export function validateExtent(extent: Uint8Array): boolean {
  try {
 ...
 * ```
 */
export function getPresentPageCount(extent: Uint8Array): number {
  const header = parseExtentHeader(extent)
  const bitmap = getPageBitmap(extent)
 ...
 * ```
 */
export function getPresentPageIndices(extent: Uint8Array): number[] {
  const header = parseExtentHeader(extent)
  const bitmap = getPageBitmap(extent)
 ...
 * ```
 */
export function isExtentCompressed(extent: Uint8Array): boolean {
  const header = parseExtentHeader(extent)
 ...
 * ```
 */
export function calculateBitmapSize(pageCount: number): number {
 ...
 * ```
 */
export function calculateExtentSize(
  pageCount: number,
 ...
 * ```
 */
export function calculatePagesPerExtent(
  extentSize: number,
  pageSize: number
): number {
 ...
 * ```
 */
export function setPagePresent(bitmap: Uint8Array, pageIndex: number): void {
  const byteIndex = Math.floor(pageIndex / 8)
  const bitIndex = pageIndex % 8
 ...
 * ```
 */
export function clearPagePresent(bitmap: Uint8Array, pageIndex: number): void {
  const byteIndex = Math.floor(pageIndex / 8)
  const bitIndex = pageIndex % 8
  if (byteIndex < bitmap.length) {
    const currentByte = bitmap[byteIndex]
 ...
 * ```
 */
export function isPagePresentInBitmap(
  bitmap: Uint8Array,
  pageIndex: number
): boolean {
  const byteIndex = Math.floor(pageIndex / 8)
 ...
 * ```
 */
export function countPresentPagesInBitmap(bitmap: Uint8Array): number {
  let count = 0
 ...
 */
export function parseExtent(extent: Uint8Array): ParsedExtent {
  // Parse header
  const header = parseExtentHeader(extent)
 ...
 */
export function getExtentInfo(extent: Uint8Array): {
  header: ExtentHeader
  bitmapSize: number
  isSparse: boolean
  isCompressed: boolean

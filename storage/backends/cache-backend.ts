 ...
 * ```
 */
export class CacheBlobStorage implements BlobStorage {
   ...
   */
  private extractMetadata(response: Response): BlobReadResult['metadata'] {
   ...
   */
  async list(_options?: BlobListOptions): Promise<BlobListResult> {
    // Cache API does not support listing
    // Return empty result rather than throwing to maintain compatibility
    console.warn(
      'CacheBlobStorage.list() called but Cache API does not support listing. ' +
 ...
 * ```
 */
export function createCacheBackend(config: CacheBackendConfig): BlobStorage {

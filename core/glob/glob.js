...
async function _traverse(dir, options, collector) {
 ...
 * ```
 */
function patternExplicitlyMatchesDot(pattern) {
 ...
 */
function analyzePattern(pattern, dot) {
    ...
    const segments = pattern.split('/');
    const requiredPrefixSegments = [];
    for (const seg of segments) {
        if (seg === '**' || hasWildcards(seg))
            break;
 ...
 */
function couldContainMatches(dirPath, patterns, dot) {
    ...
    for (const info of patterns) {
        ...
        if (info.hasGlobstar) {
            ...
            if (info.literalPrefix.startsWith(dirPath + '/') ||
                info.literalPrefix === dirPath ||
                dirPath.startsWith(info.literalPrefix + '/') ||
                dirPath.startsWith(info.literalPrefix)) {
                return true;
...
export async function glob(pattern, options) {
     ...
     */
    function shouldTraverseDirectory(relativeDirPath) {
        ...
        for (const info of patternInfos) {
            ...
            if (info.hasGlobstar) {
                ...
                if (relativeDirPath === info.literalPrefix ||
            ...
            if (info.matcher(deepTestFile))
                return true;
        }
        return false;
    }
    // Recursive collector function with depth tracking and early pruning (uses backend)
    async function collectEntriesWithBackend(dir, currentDirDepth, relativeDirPath) {
        ...
        let entries;
        try {
            entries = await backend.readdir(dir, { withFileTypes: true });
        }
        catch (err) {
        ...
        for (const entry of entries) {
            ...
            const entryType = stats.isDirectory() ? 'directory' : stats.isSymbolicLink?.() ? 'symlink' : 'file';
            // Handle symlinks
    ...
    async function collectEntriesWithMock(dir, currentDirDepth, relativeDirPath) {
        // Check for timeout/abort at each directory
        checkTimeout();
        checkAbort();
        const entries = mockFS.get(normalizePath(dir));
 ...
 * ```
 */
export async function* globStream(pattern, options) {
    const startTime = Date.now();
    // Normalize pattern to array
    const patterns = Array.isArray(pattern) ? pattern : [pattern];
    // Handle empty array or empty string
     ...
     */
    function checkMatch(relativePath, entryType) {
        // Check type filters
    ...
    async function* traverseWithBackend(dir, currentDirDepth, relativeDirPath) {
        checkTimeout();
        checkAbort();
        let entries;
        try {
    ...
    async function* traverseWithMock(dir, currentDirDepth, relativeDirPath) {
        checkTimeout();
        checkAbort();
        const entries = mockFS.get(normalizePath(dir));
        if (!entries)

 ...
 * @internal
 */
function isBinaryContent(data, checkSize = BINARY_CHECK_SIZE) {
 ...
 * @internal
 */
async function getFilesWithBackend(dir, recursive, backend, checkTimeout, checkAbort) {
    ...
    let entries;
    try {
        entries = await backend.readdir(normalizedDir, { withFileTypes: true });
    }
    catch (err) {
 ...
 * @internal
 */
function searchFileContent(file, lines, searchRegex, options, checkTimeout, checkAbort) {
...
export async function grep(options) {
    ...
    const { pattern, path = '/', recursive = false, glob: globPattern, ignoreCase = false, before, after, maxCount, filesOnly = false, invert = false, wordMatch = false, backend, timeout, signal, } = options;
    // -------------------------------------------------------------------------
    ...
    for (const file of filesToSearch) {
        ...
        const { matches: fileMatches, hasMatches } = searchFileContent(file, lines, searchRegex, { before, after, maxCount, filesOnly, invert }, checkTimeout, checkAbort);

 ...
 * @internal
 */
function matchesSize(size, sizeFilter) {
 ...
 * @internal
 */
function matchesTime(timestamp, timeFilter) {
 ...
 * @internal
 */
function shouldPrune(name, prunePatterns) {
 ...
 * @internal
 */
function matchesName(filename, nameFilter) {
 ...
 * @internal
 */
function _getRelativeDepth(path, startPath) {
    const normalizedPath = normalizePath(path);
    const normalizedStart = normalizePath(startPath);
    if (normalizedPath === normalizedStart)
        return 0;
...
export async function find(options = {}) {
     ...
     */
    async function traverseWithBackend(currentPath, depth) {
        ...
        const entryType = stats.isDirectory()
            ? 'directory'
     ...
     * @param depth - Current depth relative to start path
     */
    function traverseWithMock(currentPath, depth) {
        // === EARLY TERMINATION CHECKS (most efficient) ===
        // Check for timeout/abort at each iteration
        checkTimeout();
        checkAbort();

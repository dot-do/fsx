...
export class BatchEmitter {
     ...
     */
    queue(type, path, oldPath, metadata) {
        const now = Date.now();
        ...
        if (this.compressEvents) {
            ...
            if (existing) {
                ...
                const coalescedEvent = createWatchEvent(coalescedType, type === 'rename' ? (oldPath || path) : path, type === 'rename' ? path : metadata, type === 'rename' ? metadata : undefined);
     ...
     */
    coalesceTypes(existingType, newType) {
        // Delete always wins
        if (newType === 'delete')
            return 'delete';
 ...
 */
export function createBatchEmitter(options) {

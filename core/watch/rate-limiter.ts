...
/**
 * Internal tracking data for a connection
 */
interface ConnectionRateData {
  /**
   * Timestamps of messages in the main window
   */
  timestamps: number[]

  /**
   * Timestamps of messages in the burst window
   */
  burstTimestamps: number[]

  /**
   * When this connection data was last accessed (for cleanup)
   */
  lastAccess: number
}
...
export class RateLimiter {
  ...
  private readonly enableBurstProtection: boolean
  private readonly burstMaxMessages: number
  private readonly burstWindowMs: number

  /**
   * Rate data per connection (keyed by WebSocket instance)
   */
  private connectionData: Map<WebSocket, ConnectionRateData> = new Map()
   ...
   */
  getStatus(ws: WebSocket): { remaining: number; resetMs: number; limit: number } {
    const now = Date.now()
   ...
   */
  cleanup(maxAgeMs: number = RateLimiterDefaults.MAX_DATA_AGE_MS): void {
    const now = Date.now()
   ...
   */
  startCleanup(intervalMs: number = RateLimiterDefaults.CLEANUP_INTERVAL_MS): void {

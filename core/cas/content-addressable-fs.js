 ...
 * ```
 */
import { putObject as putObjectFn, putObjectBatch as putObjectBatchFn, } from './put-object.js';
import { getObject as getObjectFn } from './get-object.js';
import { hashToPath } from './path-mapping.js';
import { InMemoryRefCountStorage, calculateStats, } from './refcount.js';
import { LRUCache } from './cache.js';
import { createExistenceCache, } from './existence-cache.js';
/**
...
export class ContentAddressableFS {
    ...
    constructor(storage, options) {
        ...
        if (options?.existenceCache) {
            const existenceCacheOptions = typeof options.existenceCache === 'boolean' ? {} : options.existenceCache;
     ...
     * ```
     */
    async putObjectBatch(items, options = {}) {
        ...
        const wrappedOnProgress = async (progress) => {
    ...
    async getObject(hash) {
        ...
        if (!this.isValidHash(hash)) {
            throw new Error(`Invalid hash: expected 40 (SHA-1) or 64 (SHA-256) hex characters, got ${hash.length} characters`);
        }
     ...
     * ```
     */
    async hasObjectBatch(hashes, options = {}) {
        ...
        const results = new Array(normalizedHashes.length);
        // Track which hashes need storage check
        const needsStorageCheck = [];
     ...
     * ```
     */
    clearExistenceCache(clearBloomFilter = false) {
     ...
     * ```
     */
    async deleteObjectBatch(hashes, options = {}) {
        const { concurrency = 10, onProgress } = options;
        const results = [];
        ...
        for (let i = 0; i < totalItems; i += concurrency) {
            ...
            const batchResults = await Promise.all(batch.map(async (hash, batchIndex) => {
...
//# sourceMappingURL=content-addressable-fs.js.map